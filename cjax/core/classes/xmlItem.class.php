		...
		if(is_a($value,__class__) || is_a($value,'plugin')) {
			...
			switch($setting) {
				...
				case 'callback':
					...
					$callback = CoreEvents::$cache[$value->id];
					
					$event['callback'][$value->id] = $callback;
					$value->delete();
					
					$callbacks = CoreEvents::processScache($event['callback']);
					$callbacks = CoreEvents::mkArray($callbacks,'json', true);
					$event['callback'] =  "<callback><cjax>$callbacks</cjax></callback>";
