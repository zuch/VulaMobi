...
class CoreEvents extends cjaxFormat {
	...
	function out()
	{
		...
		$out  = "<xml class='cjax'>".$_cache."</xml><preload>$_preload</preload>";
		if(self::$wrapper) {
	...
	function _processScachePlugin($v,$caller = null)
	{
		...
		if(isset($v['callback'])) {
			$v['callback'] = '<callback>'.self::mkArray($v['callback']).'</callback>';
		}	
...
if (document.addEventListener) {
	...
	{
		$json = self::encode(json_encode($array,JSON_FORCE_OBJECT));
	...
	{
		$ajax = ajax();
		$file = "cjax-5.0-Stable.js";
		if($min) {
			$file = "cjax-5.0-Stable.min.js";
		}
		...
		if($this->jsdir) {
			$ajax->_path = $js_path;
			$script[] = "<script defer='defer' id='cjax_lib' type='text/javascript' src='{$js_path}{$file}'></script>\n";
		} else if(self::$path) {
	...
	function isAjaxRequest()
	{
		...
		if(!empty($headers)) {
			if($headers['X-Requested-With']=='CJAX FRAMEW0RK 5.0-Stable' || $headers['X-Requested-With']=='XMLHttpRequest') {
				return true;
			}
		}
